...
bool ShadingCorrection::CalcMaskBGR(cv::Mat& src, cv::Mat& mask, cv::Scalar& global_mean) {
    ...
    mask = cv::Mat::ones(src.rows, src.cols, CV_32FC3);
...
int main() {
    ...
    cv::Mat image = cv::imread("C:\\WorkSpace\\mytest\\tjh_test\\opencv_test\\testfile1.png");
    ...
    corrector.ApplyMaskToImage(invertedImage, invertedMask, correctedImage);
